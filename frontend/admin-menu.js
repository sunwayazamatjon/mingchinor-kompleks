...
function renderItemsList() {
    ...
    container.innerHTML = filtered.map(item => {
        const name = item.name_uz || item.name || 'Noma\'lum';
        const desc = item.desc_uz || item.desc || '';
        const cat = item.category_uz || item.category || '—';
        return `
        <div class="table-row-item table-col-7 ${!item.available ? 'unavailable' : ''}" onclick="selectForEdit(${item.id})">
            ...
            <div>
                <strong>${item.emoji} ${name}</strong><br>
                <small style="color:#64748b;">${desc.substring(0,45)}</small><br>
...
function selectForEdit(id) {
    ...
    const name = item.name_uz || item.name;
    document.getElementById('selectedItemInfo').innerHTML = `
        ...
        <div><strong>${item.emoji} ${name}</strong><br><span style="font-size:12px;color:#64748b;">ID: #${item.id}</span></div>
    `;
    ...
    document.getElementById('editItemName_uz').value = item.name_uz || item.name || '';
    document.getElementById('editItemName_ru').value = item.name_ru || '';
    document.getElementById('editItemName_en').value = item.name_en || '';
    
    document.getElementById('editItemCategory_uz').value = item.category_uz || item.category || '🍜 Birinchi taomlar';
    document.getElementById('editItemCategory_ru').value = item.category_ru || '🍜 Первые блюда';
    document.getElementById('editItemCategory_en').value = item.category_en || '🍜 Main Soups';
    
    document.getElementById('editItemDesc_uz').value = item.desc_uz || item.desc || '';
    document.getElementById('editItemDesc_ru').value = item.desc_ru || '';
    document.getElementById('editItemDesc_en').value = item.desc_en || '';
...
async function updateSelectedItem() {
    ...
    item.name_uz = document.getElementById('editItemName_uz').value.trim();
    item.name_ru = document.getElementById('editItemName_ru').value.trim();
    item.name_en = document.getElementById('editItemName_en').value.trim();

    item.category_uz = document.getElementById('editItemCategory_uz').value;
    item.category_ru = document.getElementById('editItemCategory_ru').value;
    item.category_en = document.getElementById('editItemCategory_en').value;

    item.desc_uz = document.getElementById('editItemDesc_uz').value.trim();
    item.desc_ru = document.getElementById('editItemDesc_ru').value.trim();
    item.desc_en = document.getElementById('editItemDesc_en').value.trim();

    // Migratsiya uchun eski maydonlarni ham yangilab qo'yamiz
    item.name = item.name_uz;
    item.category = item.category_uz;
    item.desc = item.desc_uz;
    ...
    if (fileInput.files && fileInput.files[0]) {
        readImageAsBase64(fileInput.files[0], async base64 => {
            ...
            renderItemsList();
            cancelEdit();
            showToast(`✅ "${item.name_uz}" tahrirlandi!`);
        });
        return;
    }
    await saveMenuToAPI();
...
async function toggleAvailability(id) {
    ...
    if (item) {
        ...
        await saveMenuToAPI();
        renderItemsList();
        showToast(`${item.name_uz || item.name} ${item.available ? '✅ yoqildi' : '⏸️ o\'chirildi'}`);
    }
}
...
async function deleteItem(id) {
    ...
    if (confirm(`"${item?.name}" ni o'chirmoqchimisiz?`)) {
        ...
        showToast(`❌ "${item?.name_uz || item?.name}" o'chirildi`);
    }
}
...
async function addNewItem() {
    ...
    const name_uz = document.getElementById('itemName_uz').value.trim();
    const name_ru = document.getElementById('itemName_ru').value.trim();
    const name_en = document.getElementById('itemName_en').value.trim();

    const category_uz = document.getElementById('itemCategory_uz').value;
    const category_ru = document.getElementById('itemCategory_ru').value;
    const category_en = document.getElementById('itemCategory_en').value;

    const desc_uz = document.getElementById('itemDesc_uz').value.trim();
    const desc_ru = document.getElementById('itemDesc_ru').value.trim();
    const desc_en = document.getElementById('itemDesc_en').value.trim();

    if (!name_uz || isNaN(price) || price <= 0) {
        alert('Taom nomi (UZ) va narxini to\'g\'ri kiriting!');
        return;
    }
    ...
    const newItem = { 
        id: newId, 
        emoji, 
        name: name_uz, // Eskilar bilan moslik uchun
        name_uz, name_ru, name_en,
        category: category_uz, 
        category_uz, category_ru, category_en,
        desc: desc_uz,
        desc_uz, desc_ru, desc_en,
        price, cost, available: true, image: '' 
    };
...
function clearAddForm() {
    ['itemEmoji','itemPrice','itemCost','itemImageUrl',
     'itemName_uz','itemName_ru','itemName_en',
     'itemDesc_uz','itemDesc_ru','itemDesc_en'
    ].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.value = '';
    });
